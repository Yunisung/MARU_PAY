...
public class KspayV14 {
	...
	public KspayV14(String cid,String device) {
		this.cid = cid;
	...
	private String comm(String action){
		...
		req.append("sndCommConId=").append(cid).append("&sndActionType=").append(action).append("&sndRpyParams=").append(URLEncode(KspayV14.PARAM));
		...
		try {
			logger.info("kspayV14 send : [{}]",req.toString());
			...
			os.write(req.toString().getBytes("euc-kr"));
			os.flush();
			os.close();
			...
			result ="CONNECT ERROR ["+e.getMessage()+"] "+KspayV14.KSPAY_WEB_URL;
		}finally{
			logger.info("ElapsedTime : {} msec",(long)(System.currentTimeMillis()-time));
			logger.info("kspayV14 recv : [{}]",result.trim());
			conn.disconnect();
		}
	...
	private SharedMap<String,String> getValue(String sText){
		...
		if(sText.indexOf("`") == -1){
			...
			String[] values = CommonUtil.split(sText, "[`]", true, KspayV14.PARAMS.length);
			for(int i=0;i<KspayV14.PARAMS.length;i++){
				resMap.put(KspayV14.PARAMS[i], CommonUtil.nToB(values[i]).trim());
			}
		}
		return resMap;
	}
