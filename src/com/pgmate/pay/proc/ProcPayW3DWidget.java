 ...
 *
 */
public class ProcPayW3DWidget extends Proc {
	private static Logger logger 	= LoggerFactory.getLogger( com.pgmate.pay.proc.ProcPayW3DWidget.class );
	private String widgetKey 		= "";			
	
	public ProcPayW3DWidget() {
	}
	...
	public void setKspay(SharedMap<String,Object> vanMap){
		...
		request.widget.put("targetUrl", "https://kspay.ksnet.to/abroadMPI/veri_host.jsp");
		
		request.widget.put("width", 500);
		String goodname = getProduct(request.widget.get("products"));
		request.widget.put("goodname", goodname);
		...
		form.put("storepasswd", "");
		form.put("authty", "M"); 	//승인타입 : A-인증없는승인, N-인증승인, M-Visa3D인증승인
		form.put("expdt", "4912"); 	
		form.put("email", request.widget.getString("payerEmail")); 	
		form.put("phoneno", request.widget.getString("payerTel").replaceAll("[-]", "")); 	
		form.put("interest", ""); 	
		form.put("interesttype", ""); 	
		form.put("ordernumber", sharedMap.getString(PAYUNIT.TRX_ID)); 	
		form.put("ordername", request.widget.getString("payerName")); 	
		form.put("idnum", "");		// 주민등록번호
		form.put("goodname", goodname);		
		form.put("amount", request.widget.getString("amount"));		
		form.put("currencytype", "WON");		
		form.put("interestname", "");
		
		form.put("installment", mchtTmnMap.getInt("apiMaxInstall"));		
		form.put("xid", "");		
		form.put("eci", "");		
		form.put("cavv", "");	 	
		form.put("cardno", "");	 	
...
//		}
		...
		form.put("url", "http://www.ghpayments.co.kr");	// 광원 사업자 url로 고정한다.
		...
		if(sharedMap.isEquals(PAYUNIT.RUNTIME_ENV, PAYUNIT.RUNTIME_ENV_LIVE)){
			...
			form.put("returnUrl", String.format("http://%s%s/%s/%s",PAYUNIT.PAY_HOST_DEV,PAYUNIT.API_W3D_HOOK,vanMap.getString("van"),sharedMap.getString(PAYUNIT.TRX_ID)));
		}
		
		// Visa3d인증에 사용될 인자들
		String wmpi_hostid	= "00000111";
		String wmpi_hostpwd	= "ksnetdkagh";

		form.put("wmpi_hostid", wmpi_hostid);
		form.put("wmpi_hostpwd", wmpi_hostpwd);
		
		int		mpiExponent	= 0;
		String	mpiCurrency	= "410";
		//String	mpiAmount	= recalcamount;
	    String mpiAmount	= form.getString("amount");
	    ...
	    if(currencytype.equals("USD"))  
		{
			mpiExponent = 2;
			mpiCurrency	= "840";
			mpiAmount	= form.getString("amount").substring(0,form.getString("amount").length()-1); //소수점 2자리만 사용하므로 마지막 한자리는 버린다.
		} 
		*/
		form.put("mpiExponent", mpiExponent);
		form.put("mpiCurrency", mpiCurrency);
		form.put("mpiAmount", mpiAmount);
