...
//			if(vactDtlMap.getString("status").equals("발행") || vactDtlMap.getString("status").equals("대기")){
			if(vactDtlMap.getString("status").equals("발행")){
				...
				vact.status 	= "사용자만료";
				if(trxDAO.updateVactDtlClose(vactDtlMap)) {
					logger.info("account closing : {} ", vactDtlMap.getString("account"));
					trxDAO.insertHtVactDtl(vactDtlMap.getString("issueId"), "0000", "가상계좌가 사용자 만료처리 되었습니다.");
					response.result = ResultUtil.getResult("0000", "계좌만료", "가상계좌가 만료처리 되었습니다. " + vactDtlMap.getString("status") + " -> 사용자만료");
					return;
				}
			}else if(vactDtlMap.getString("status").equals("대기")){
				logger.info("account 대기상태이므로 사용자만료가 불가능합니다.");
				response.result = ResultUtil.getResult("9999", "계좌만료","대기 상태인 가상계좌입니다. "+vactDtlMap.getString("status"));return;
			}else{
				logger.info("account aleady closed");
				response.result = ResultUtil.getResult("9999", "계좌만료","이미 만료된 가상계좌입니다. "+vactDtlMap.getString("status"));return;
			}
