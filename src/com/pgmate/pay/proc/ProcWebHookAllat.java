 ...
 *
 */
public class ProcWebHookAllat {
	private static Logger logger = LoggerFactory.getLogger(com.pgmate.pay.proc.ProcWebHookAllat.class);
	...
	public void exec(RoutingContext rc,SharedMap<String,Object> sharedMap) {
		...
		logger.debug("PAYLOAD: "+sharedMap.getString(PAYUNIT.PAYLOAD));
		...
		payLoadMap.put("trxDate",sharedMap.getString(PAYUNIT.REG_DATE));
		if(requestMap.getString("sign").equals("-")){
			payLoadMap.put("trxType","REFUND");
		} else {
			payLoadMap.put("trxType","PAY");
		}
		payLoadMap.put("trackId",sharedMap.getString("trackId"));
		payLoadMap.put("vanTrxId",sharedMap.getString("vanTrxId"));
		payLoadMap.put("authCd",CommonUtil.nToB(requestMap.getString("approval_no")));
		payLoadMap.put("cardType",sharedMap.getString("cardType"));
		...
		payLoadMap.put("installment",CommonUtil.nToB(requestMap.getString("sell_mm")));
		payLoadMap.put("amount",sharedMap.getString("amount"));
			
		String payLoad = CommonUtil.toQueryString(payLoadMap,"UTF-8");
		sharedMap.put(PAYUNIT.PAYLOAD, payLoad);
	...
	private boolean valid() {
		...
		if(requestMap.getString("REQ_TYPE").equals("PAY")){
			...
			if (trxDAO.isDuplicatedVanTrxId(sharedMap.getString("van"),requestMap.getString("tx_seq_no"))) {
			...
			if (trxDAO.isDuplicatedRFDVanTrxId(sharedMap.getString("van"),requestMap.getString("tx_seq_no"))) {
		...
		if(requestMap.getString("REQ_TYPE").equals("PAY")){
			...
			String tmnId = trxDAO.getOffPgTmnIdByMid(requestMap.getString("shop_id"),requestMap.getString("approval_no"),rootTrxDay,requestMap.getLong("amt"));
			logger.debug("OFFPG TmnId [{}]",tmnId);
			
			if(CommonUtil.isNullOrSpace(tmnId)){
				// 일반 단말기 결제
				mchtTmnMap = trxDAO.getMchtTmnByVanId(requestMap.getString("shop_id"));
			} else {
				// 다중사용자 단말기 거래
...
//				try{Thread.sleep(1000);}catch(Exception e){}
			...
			if(requestMap.getLong("amt") == trxMap.getLong("amount")){
		...
		// 승인거래의 경우 다음을 확인한다.
			...
			SharedMap<String, Object> vanIdMap = trxDAO.getVanByVanId("ALLAT", requestMap.getString("shop_id"));
