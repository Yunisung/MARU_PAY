...
public class ProcPaycoMobileReturn extends Proc{
    private static Logger logger = LoggerFactory.getLogger( ProcPaycoMobileReturn.class );
    private SharedMap<String,Object> ioMap =  null;
    String trxId = "";
    public ProcPaycoMobileReturn() {
    ...
    public void exec(RoutingContext rc, Request request, SharedMap<String, Object> sharedMap, SharedMap<String, SharedMap<String, Object>> sharedObject) throws Exception {
        ...
        //소켓통신에 필요한 데이터 세팅
        String search = sharedMap.getString(PAYUNIT.URI).replaceAll(PAYUNIT.API_PAYCO_MOBILE_RETURN+"/", "");
        logger.info("PAYCO_MOBILE_RETURN : [{}]",search);
        ...
        payco.setXtrno("KS" + new java.text.SimpleDateFormat("yyyyMMddHHmmss").format(new java.util.Date()));
...
// Server로 부터 응답이 없을시 자체응답
    String rApprovalType     = "1001";          //승인구분
...
// --------------------------------------------------------------------------------
    ...
    try
    {
        ...
        if(certitype.equals("MOBILE"))  // payco
        {
            ApprovalType      = "1000" ;
            InterestType      = "1" ;
            CurrencyType      = payco.getCurrencytype() ;
            Installment       = payco.getInstallment() ;
            TrackII           = "PAYCO";
            AuthType          = "S";
            MPIPositionType   = "K" ;
            MPIReUseType      = "N" ;
        ...
        // 통화구분 0:원화 1: 미화
        if (CurrencyType.equals("USD")||CurrencyType.equals("840"))	CurrencyType = "1";
        else	CurrencyType = "0" ;
...
//        String name = "%EB%B0%3F%3F%A4%EC%3F%3F";
        ...
        String utf = new ProcPaycoMobileReturn().changeCharset(name, "UTF-8");
        String euc = new ProcPaycoMobileReturn().changeCharset(name, "EUC-KR");
