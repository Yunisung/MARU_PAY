...
public class VactRegV2 extends Proc {
    ...
    /*
    PYS : 등록을 하고 들어오기 때문에, 등록관련 로직을 전부 제거 한다 (+인증로직)
...
//                bean.resultMsg = "테스트해보기";
                ...
                if("0000".equals(bean.resultCd)) {
                    ...
                    if("0".equals(trxType)) {
                        ...
                        response.result = ResultUtil.getResult("0000", "정상","가상계좌 출금정보가 등록되었습니다.");
                        ...
                        SharedMap<String,Object> dtl = trxDAO.getVactDtl(issueId);

                        vact = new SharedMap<String,Object>();
                        vact.put("issueId", issueId);
                        vact.put("status","발행");
                        vact.put("holderName", request.vact.holderName);
                        //PYS : 출근계좌등록하면 영구계좌로 바뀌기 때문에 아래와 같이 처리
                        vact.put("amount", dtl.get("amount"));
                        vact.put("oper", dtl.get("oper"));
