 ...
 *
 */
public class ProcPay3DWidgetMobile extends Proc {
	private static Logger logger 	= LoggerFactory.getLogger( com.pgmate.pay.proc.ProcPay3DWidgetMobile.class );
	private String widgetKey 		= "";			
	
	public ProcPay3DWidgetMobile() {
	}
	...
	public void valid() {
		...
		if(sharedMap.getString(PAYUNIT.METHOD).equalsIgnoreCase("POST")){
			...
			if(request.widget == null){
				...
				//KJM : 일반 3D(온라인결제)와 달리 가맹점등 한도 조회 안함
				//KJM : ioMap에 res 데이터 넣은 뒤 3D위젯 테이블에 등록
		...
		//KJM : get 요청일 때
			...
			String key = sharedMap.getString(PAYUNIT.URI).replaceAll(PAYUNIT.API_3D_MOBILE_WIDGET+"/", "");
	...
	public void setKspay(SharedMap<String,Object> vanMap){
		...
		if(sharedMap.isEquals(PAYUNIT.RUNTIME_ENV, PAYUNIT.RUNTIME_ENV_LIVE)){
			form.put("sndReply", String.format("https://%s%s/%s/%s",PAYUNIT.PAY_HOST_LIVE,PAYUNIT.API_3D_MOBILE_HOOK,vanMap.getString("van"),sharedMap.getString(PAYUNIT.TRX_ID)));
		}else{
			form.put("sndReply", String.format("http://%s%s/%s/%s",PAYUNIT.PAY_HOST_DEV,PAYUNIT.API_3D_MOBILE_HOOK,vanMap.getString("van"),sharedMap.getString(PAYUNIT.TRX_ID)));
		}
	...
	public void setDaou(SharedMap<String,Object> vanMap){
		...
		if(sharedMap.isEquals(PAYUNIT.RUNTIME_ENV, PAYUNIT.RUNTIME_ENV_LIVE)){
			form.put("RETURNURL", String.format("https://%s/%s/%s/%s",PAYUNIT.PAY_HOST_LIVE,PAYUNIT.API_3D_MOBILE_HOOK,vanMap.getString("van"),sharedMap.getString(PAYUNIT.TRX_ID)));
		}else{
			form.put("RETURNURL", String.format("http://%s/%s/%s/%s","cp.cyrexpay.com:8080",PAYUNIT.API_3D_MOBILE_HOOK,vanMap.getString("van"),sharedMap.getString(PAYUNIT.TRX_ID)));
		}
