...
public class ProcLpayMobileReturn extends Proc{
    private static Logger logger = LoggerFactory.getLogger( com.pgmate.pay.proc.ProcLpayMobileReturn.class );
    private SharedMap<String,Object> ioMap =  null;
    String trxId = "";

    public ProcLpayMobileReturn() {
    ...
    public void exec(RoutingContext rc, Request request, SharedMap<String, Object> sharedMap, SharedMap<String, SharedMap<String, Object>> sharedObject) throws Exception {
        ...
        //소켓통신에 필요한 데이터 세팅
        String search = sharedMap.getString(PAYUNIT.URI).replaceAll(PAYUNIT.API_LPAY_MOBILE_RETURN+"/", "");
        logger.info("LPAY_MOBILE_RETURN : [{}]",search);
        ...
        if(trxCheckMap != null) {
            ...
            TemplateUtil.simplePayResultPage(rc,"lpayMobile", URLEncode(GsonUtil.toJsonExcludeStrategies(response)));
            return;
        }
